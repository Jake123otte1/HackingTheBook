...
int main()
{
    ...
    char_ptr = char_array; // The character pointer points to the first memory address of the character array.
    int_ptr = int_array; // The integer pointer points to the first memory address of the integer array.

    for(int i = 0; i < sizeof(char_array); i++)
    {
        printf("[Integer Pointer]\t points to address %p, which contains the integer %d\n", int_ptr, *int_ptr); // Prints the pointer's value, a memory address, and the value that the memory address holds.
    ...
    for(int i = 0; i < sizeof(char_array); i++)
    {
        printf("[Character Pointer]\t points to address %p, which contains the character %c\n", char_ptr, *char_ptr); // Ditto as above.
